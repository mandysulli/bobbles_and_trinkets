...
try:
    irma_path, variant_xlsx, summary_csv = argv[1], argv[2], argv[3]
except IndexError:
    exit(
...
ha_df['HA_subtype'] = np.where(ha_df.Files.str.contains("A_HA_H1.bam"), "H1",
                   ...
                   np.where(ha_df.Files.str.contains("B_HA.bam"), "BVIC", "Undetermined"))))

na_df['NA_subtype'] = np.where(na_df.Files.str.contains("A_NA_N1.bam"), "N1",
                   np.where(na_df.Files.str.contains("A_NA_N2.bam"), "N2",
                   np.where(na_df.Files.str.contains("B_NA.bam"), "BVIC", "Undetermined")))
...
combined['Subtype'] = combined['Subtype'].str.replace(r'Undetermined', 'Undetermined', regex=True)
